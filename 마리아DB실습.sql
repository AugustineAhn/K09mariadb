...
(
    ...
    name varchar(30) not null, -- 10바이트가 아니고 10글자쓸수있음 n을 쓰면 
    regidate datetime default CURRENT_TIMESTAMP, /*현재시간을 디폴트로사용
	 												datetime: 날짜와 시간을 동시에 표현할 
													 수 있는 자료형*/
    primary key(id) 
); 
...
# 게시판 테이블(자식)
/*
	AUTO INCREMENT :오라클의 시퀀스를 대체할 수 있는 속성으로
...
INSERT INTO tb_int (num1, num2, num3, num4, fnum1, fnum2)
	VALUES (100, 12345, 1234567, 12345678990, 
...
INSERT INTO tb_int (idx, num1, num2, num3, num4, fnum1, fnum2)
	VALUES (2, 100, 12345, 1234567, 12345678990, 
		12345.12345, 1234567890.1234567891);
...
/*
	날짜타입의 컬럼에 현재 날짜를 입력할때 오라클은 sysdate를
	사용하자만 MySQL은 now()함수를 사용한다.
*/
...
/*
 페이지 처리를 위해 게시물의 범위를 정할때 Oracle은 rawnum의
 속성을 사용하지만 MariaDB는 limit을 사용한다.
...
INSERT INTO board (title, content, id, bname)
	VALUE	('여긴 자유게시판', '내용없음', 'kosmo', 'freeboard');
INSERT INTO board (title, content, id, bname)
	VALUE	('여긴 질문과답변', '내용없음', 'kosmo', 'qna');	
	
INSERT INTO board (title, content, id, bname)
	VALUE	('여긴 FAQ', '내용없음', 'kosmo', 'faq');
-- 자유게시판 리스트 보기
